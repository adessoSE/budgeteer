...
public interface BudgetRepository extends CrudRepository<BudgetEntity, Long> {
  ...
  @Query(
      "select new de.adesso.budgeteer.persistence.budget.MissingBudgetTotalBean(b.id, b.name) from BudgetEntity b where b.total = de.adesso.budgeteer.common.old.MoneyUtil.ZERO and b.project.id=:projectId order by b.name")
   ...
   */
  @Query(
      "select new de.adesso.budgeteer.persistence.budget.MissingBudgetTotalBean(b.id, b.name) from BudgetEntity b where b.total = de.adesso.budgeteer.common.old.MoneyUtil.ZERO and b.id=:budgetId order by b.name")
  MissingBudgetTotalBean getMissingBudgetTotalForBudget(@Param("budgetId") long budgetId);

  @Query(
      "select new de.adesso.budgeteer.persistence.budget.LimitReachedBean(b.id, b.name) from BudgetEntity b where b.limit <= :spent and b.limit > de.adesso.budgeteer.common.old.MoneyUtil.ZERO and b.id=:budgetId order by b.name")
  LimitReachedBean getLimitReachedForBudget(
      @Param("budgetId") long budgetId, @Param("spent") Money spent);

  @Query(
      "select new de.adesso.budgeteer.persistence.budget.LimitReachedBean(b.id, b.name) from BudgetEntity b where b.project.id=:projectId order by b.name")
