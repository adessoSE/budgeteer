...
public class InvoiceEntityAdapter
    implements GetInvoicesInProjectPort,
  ...
  public Invoice createInvoiceEntity(CreateInvoiceEntityCommand command) {
    var contractEntity =
        contractRepository.findById(command.getContractId()).orElseThrow(RuntimeException::new);
  ...
  public Invoice updateInvoice(UpdateInvoiceEntityCommand command) {
    var invoiceEntity =
        invoiceRepository.findById(command.getInvoiceId()).orElseThrow(RuntimeException::new);
    var contractEntity =
        contractRepository.findById(command.getContractId()).orElseThrow(RuntimeException::new);
  ...
  private void addDynamicFields(
      Map<String, String> attributes, ContractEntity contractEntity, InvoiceEntity invoiceEntity) {
    attributes.forEach(
        (field, value) -> {
  ...
  public boolean userHasAccessToInvoice(String username, long invoiceId) {
    return contractAdapter.userHasAccessToContract(
        username,
        invoiceRepository
            .findById(invoiceId)
            .map(InvoiceEntity::getContract)
