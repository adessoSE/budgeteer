...
public class PersonMapper {
  ...
  public Person mapToPerson(PersonEntity personEntity) {
    return new Person(
        ...
        calculateAvarageDailyRate(personEntity.getWorkRecords()),
        findFirstBooked(personEntity.getWorkRecords()),
        findLastBooked(personEntity.getWorkRecords()),
        personEntity.getDefaultDailyRate(),
        calculateHoursBooked(personEntity.getWorkRecords()),
  ...
  public PersonWithRates mapToPersonWithRates(PersonEntity personEntity) {
    return new PersonWithRates(
        personEntity.getId(),
        personEntity.getName(),
        personEntity.getImportKey(),
        personEntity.getDefaultDailyRate(),
        personRateMapper.mapToDomain(personEntity.getDailyRates()));
  }
  ...
  private Double calculateHoursBooked(List<WorkRecordEntity> workRecords) {
    return workRecords.stream().mapToDouble(WorkRecordEntity::getHours).sum();
  }
        ...
        .map(
            workRecordEntity -> {
              ...
              return workRecordEntity
                  .getDailyRate()
                  .multipliedBy(workHourCoefficient, RoundingMode.HALF_DOWN);
            })
        .reduce(Money.zero(CurrencyUnit.EUR), Money::plus);
  }
}
