...
class TemplateWriterTest {
	...
	void setUp() throws Exception {
		...
		dto1.setDynamic(
				Arrays.asList(new Attribute("vorname", "Max"), new Attribute("nachname", "Mustermann")));
		...
		dto2.setDynamic(
				Arrays.asList(new Attribute("vorname", "Marina"), new Attribute("nachname", "Musterfrau")));
	...
	void testCopyRow() {
		...
		for (Cell cell : sheet.getRow(4)) {
			...
			switch (cell.getCellTypeEnum()) {
				case NUMERIC:
					assertEquals(
							cell.getDateCellValue(), copiedRow.getCell(cell.getColumnIndex()).getDateCellValue());
					assertEquals(
							cell.getNumericCellValue(),
							copiedRow.getCell(cell.getColumnIndex()).getNumericCellValue(),
							10e-8);
					break;
				case STRING:
					assertEquals(
							cell.getStringCellValue(),
							copiedRow.getCell(cell.getColumnIndex()).getStringCellValue());
					break;
				case BOOLEAN:
					assertEquals(
							cell.getBooleanCellValue(),
							copiedRow.getCell(cell.getColumnIndex()).getBooleanCellValue());
					break;
				case FORMULA:
					assertEquals(
							cell.getCellFormula(), copiedRow.getCell(cell.getColumnIndex()).getCellFormula());
					break;
				default:
