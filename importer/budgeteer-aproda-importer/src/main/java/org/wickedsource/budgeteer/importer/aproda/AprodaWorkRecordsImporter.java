...
public class AprodaWorkRecordsImporter implements WorkRecordsImporter {
  ...
  public ExampleFile getExampleFile() {
    ...
    try {
      ...
      XSSFWorkbook workbook =
          new XSSFWorkbook(getClass().getResourceAsStream("/example_aproda_report.xlsx"));
  ...
  public List<ImportedWorkRecord> read(ImportFile file)
      throws ImportException, InvalidFileFormatException {
    try {
  ...
  private boolean isImportable(Row row) {
    return row != null
        && ("ja".equalsIgnoreCase(row.getCell(COLUMN_INVOICABLE).getStringCellValue()))
