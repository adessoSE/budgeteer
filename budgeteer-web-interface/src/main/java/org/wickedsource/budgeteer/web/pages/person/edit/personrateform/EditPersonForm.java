...
public class EditPersonForm extends Form<PersonWithRates> {
  ...
  public EditPersonForm(String id, IEditPersonPageStrategy strategy) {
    ...
    this.missingDailyRateForBudgetBeans =
        personService.getMissingDailyRatesForPerson(getModelObject().getPersonId());
  ...
  private void addComponents() {
    ...
    PersonEditPanel personEditPanel =
        new PersonEditPanel(
            "newRatePanel",
            new PersonRateFormDto(
                getModelObject().getPersonId(),
                new ArrayList<>(),
          ...
          protected void rateAdded(PersonRate rate) {
            ...
            missingDailyRateForBudgetBeans.removeIf(
                missingDailyRateForBudgetBean ->
                    (missingDailyRateForBudgetBean
                            .getBudgetName()
                            .equals(rate.getBudget().getName())
                        && missingDailyRateForBudgetBean
                                .getStartDate()
                                .compareTo(rate.getDateRange().getStartDate())
                            == 0
                        && missingDailyRateForBudgetBean
                                .getEndDate()
                                .compareTo(rate.getDateRange().getEndDate())
                            == 0));
            missingDailyRateTableContainer.setVisible(false);
          }

          @Override
          protected List<PersonRate> getOverlappingRates(
            ...
            for (PersonRate p : EditPersonForm.this.getModelObject().getRates()) {
              if (p.getBudget().equals(budget)
                  && DateUtil.isDateRangeOverlapping(p.getDateRange(), dateRange)) {
                result.add(p);
              }
            }
            return result;
          ...
          public void onSubmit() {
            ...
            if (!EditPersonForm.this.hasErrorMessage()) {
              ...
              List<String> warnings =
                  personService.getOverlapWithManuallyEditedRecords(
                      EditPersonForm.this.getModelObject(), BudgeteerSession.get().getProjectId());
  ...
  private ListView<MissingDailyRateForBudgetBean> createMissingDailyRateTable() {
    ListView<MissingDailyRateForBudgetBean> missingDailyRatesListView =
        new ListView<MissingDailyRateForBudgetBean>(
  ...
  private Button createMissingDailyRateSubmitButton() {
    return new Button("fixRates") {
      ...
      public void onSubmit() {
        ...
        List<PersonRate> missingRates =
            missingDailyRateForBudgetBeans.stream()
                .map(
                    missingDailyRate ->
                        new PersonRate(
                            currentDefaultDailyRate,
                            budgetService.loadBudgetBaseData(missingDailyRate.getBudgetId()),
                            new DateRange(
                                missingDailyRate.getStartDate(), missingDailyRate.getEndDate())))
  ...
  private Money getDefaultDailyRate() {
    if ((importDefaultDailyRate.getInput() == null
        || importDefaultDailyRate.getInput().isEmpty())) {
      return null;
    }
  ...
  public void onEvent(IEvent<?> event) {
    if (event.getPayload() instanceof Notification) {
      MissingDailyRateForBudgetNotification notification =
          (MissingDailyRateForBudgetNotification) event.getPayload();
      ...
      for (OptionGroup<BudgetBaseData> e :
          budgetService.getPossibleBudgetDataForPersonAndProject(
              BudgeteerSession.get().getProjectId(), getModelObject().getPersonId())) {
        for (BudgetBaseData budget : e.getOptions()) {
          if (budget.getName().equals(budgetName)) {
            budgetData = budget;
            ((PersonEditPanel) this.get("ratesTable").get("newRatePanel"))
                .setData(
                    new PersonRateFormDto(
                        this.getModelObject().getPersonId(),
                        Collections.singletonList(budgetData),
                        Money.zero(CurrencyUnit.EUR),
                        dateRange));
          }
        }
    ...
    return new ListView<PersonRate>(id, model(from(getModel()).getRates())) {
      ...
      protected void populateItem(final ListItem<PersonRate> item) {
        ...
        item.add(
            new PersonInfoPanel(
                infoOrEditPanel,
                EditPersonForm.this.getModelObject(),
                item.getModelObject(),
                EditPersonForm.this.getModelObject().getRates()) {

              @Override
              protected ListItem<PersonRate> getEditPanel() {

                PersonRateFormDto dto =
                    new PersonRateFormDto(
                        EditPersonForm.this.getModelObject().getPersonId(),
                        Collections.singletonList(item.getModelObject().getBudget()),
                        item.getModelObject().getRate(),
                        item.getModelObject().getDateRange());
                PersonEditPanel editPanel =
                    createPanelToEditExistingPerson(infoOrEditPanel, dto, item.getModelObject());
  ...
  private PersonEditPanel createPanelToEditExistingPerson(
      String id, PersonRateFormDto dto, PersonRate previousRate) {
    return new PersonEditPanel(id, dto, true) {
      ...
      protected List<PersonRate> getOverlappingRates(DateRange dateRange, BudgetBaseData budget) {
        ...
        for (PersonRate p : getModelObject().getRates()) {
          if (!p.equals(previousRate)
              && p.getBudget().equals(budget)
              && DateUtil.isDateRangeOverlapping(p.getDateRange(), dateRange)) {
            result.add(p);
          }
        }
