...
public abstract class BasePage extends WebPage {
	...
	@SpringBean private BudgeteerSettings settings;
	...
	private void addComponents() {
		...
		notificationDropdown =
				new NotificationDropdown("notificationDropdown", new NotificationModel(projectId));
		...
		BookmarkablePageLink pageLink =
				new BookmarkablePageLink<ProjectAdministrationPage>(
						"administrationLink", ProjectAdministrationPage.class);
		add(pageLink);
	...
	private HashSet<String> loadRolesFromCurrentUser() {
		if (settings.isKeycloakActivated()) {
			...
			AccessToken accessToken =
					((KeycloakPrincipal) request.getUserPrincipal()).getKeycloakSecurityContext().getToken();
	...
	public void renderHead(IHeaderResponse response) {
		...
		response.render(
				JavaScriptReferenceHeaderItem.forReference(BudgeteerReferences.getAdminLteAppReference()));
		response.render(
		...
		return new Link(id) {
			...
			public void onClick() {
				if (settings.isKeycloakActivated()) {
					...
					setResponsePage(
							new SelectProjectPage(this.getWebPage().getClass(), new PageParameters()));
				}
			}
		};
	}
	...
	/**
	 * Expects a BreadcrumbsModel to be returned that contains all pages that should be displayed in
	 * the breadcrumbs.
	 */
