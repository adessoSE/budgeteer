...
public class SingleBudgetWeekReportPage extends BasePage {
	...
	public SingleBudgetWeekReportPage(PageParameters parameters) {
		...
		IModel<List<AggregatedRecord>> tableModel =
				new BudgetsWeeklyAggregatedRecordsModel(getParameterId());
		add(new AggregatedRecordTable("table", tableModel));

		add(
				new BudgetLinkDropdownForm("budgetLinkDropdownForm")
						.setLinkType(BudgetLinkDropdownForm.BudgetLinkType.WEEKLY));
	}
	...
	protected BreadcrumbsModel getBreadcrumbsModel() {
		return new BreadcrumbsModel(
				DashboardPage.class, BudgetsOverviewPage.class, SingleBudgetWeekReportPage.class);
	}
}
