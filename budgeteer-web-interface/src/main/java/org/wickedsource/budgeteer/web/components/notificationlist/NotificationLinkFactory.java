...
public class NotificationLinkFactory implements Serializable {
  ...
  public WebPage getLinkForNotification(
    ...
    if (notification instanceof MissingDailyRateNotification) {
      MissingDailyRateNotification missingDailyRateNotification =
          (MissingDailyRateNotification) notification;
      return new EditPersonPage(
          EditPersonPage.createParameters(missingDailyRateNotification.getPersonId()),
          backlinkPage,
          backlinkParameters);
    } else if (notification instanceof MissingBudgetTotalNotification) {
      MissingBudgetTotalNotification missingBudgetTotalNotification =
          (MissingBudgetTotalNotification) notification;
      return new EditBudgetPage(
          EditBudgetPage.createParameters(missingBudgetTotalNotification.getBudgetId()),
          backlinkPage,
          backlinkParameters,
          false);
    } else if (notification instanceof MissingContractForBudgetNotification) {
      return new EditBudgetPage(
          EditBudgetPage.createParameters(
              ((MissingContractForBudgetNotification) notification).getBudgetId()),
          backlinkPage,
          backlinkParameters,
          false);
      ...
      return new BudgetDetailsPage(
          BudgetDetailsPage.createParameters(
              ((LimitReachedNotification) notification).getBudgetId()));
    } else {
