...
public class CustomFeedbackPanel extends Panel implements IFeedback {
	...
	private final class MessageListView extends ListView<FeedbackMessage> {
		...
		protected void populateItem(final ListItem<FeedbackMessage> listItem) {
			...
			messageContainer.add(
					new AttributeAppender(
							"class", new Model<String>(getFeedbackContainerClass(message.getLevel())), " "));

			Label messageIcon = new Label("messageIcon", "");
			messageIcon.add(
					new AttributeAppender(
							"class", new Model<String>(getFeedbackIconClass(message.getLevel())), " "));
		...
		/**
		 * Returns the right style-class for the Feedbackmassage-Icon according to the Level or the
		 * message
		 *
