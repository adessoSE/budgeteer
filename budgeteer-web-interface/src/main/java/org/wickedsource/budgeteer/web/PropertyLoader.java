...
public class PropertyLoader {
	...
	/**
	 * Loads the value of the given property key from the resource bundle that belongs to the given
	 * Wicket component class.
	 *
	...
	public static String getProperty(Class<? extends Component> propertySource, String propertyKey) {
		...
		for (IStringResourceLoader stringResourceLoader : resourceLoaders) {
			resourceValue =
					stringResourceLoader.loadStringResource(propertySource, propertyKey, locale, style, null);
		...
		if (resourceValue == null) {
			throw new WicketRuntimeException(
					String.format(
							"Property %s could not be found for class %s!",
							propertyKey, propertySource.getName()));
		} else {
