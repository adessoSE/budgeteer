	...
	private ListView<Double> createBudgetList(String wicketId) {
		return new ListView<Double>(wicketId, getModel()) {
			...
			protected void populateItem(final ListItem<Double> item) {
				...
				Link<Double> link =
						new Link<Double>("link", item.getModel()) {
							@Override
							public void onClick() {
				...
				if (session.getSelectedBudgetUnit() == null
						|| item.getModelObject().equals(session.getSelectedBudgetUnit())) {
				...
				if (link.getModelObject().equals(1d)) {
					...
					unitname =
							String.format(
									getString("unitname.manday"), MoneyUtil.createMoney(item.getModelObject()));
				}
