...
public class TemplatesTable extends Panel {
  ...
  private ListView<Template> createList(
      String id, final IModel<List<Template>> model, final WebMarkupContainer table) {
    return new ListView<Template>(id, model) {
      @Override
      protected void populateItem(final ListItem<Template> item) {
        item.setOutputMarkupId(true);
        item.add(new Label("name", item.getModel().map(Template::getName)));
        item.add(new Label("description", item.getModel().map(Template::getDescription)));
        item.add(new Label("type", item.getModel().map(template -> template.getType().toString())));
        item.add(createCheckbox(item));
        item.add(
            new Link<Void>("editPage") {
              @Override
              public void onClick() {
                WebPage page =
                    new EditTemplatePage(
                        ...
                        TemplatesPage.createParameters(item.getModelObject().getId()));
  ...
  private AjaxLink createCheckbox(ListItem<Template> item) {
    ...
    final AttributeModifier starChecked =
        new AttributeModifier("class", "btn btn-default btn-sm glyphicon glyphicon-star");
    final AttributeModifier starUnchecked =
        new AttributeModifier("class", "btn btn-default btn-sm glyphicon glyphicon-star-empty");
        ...
        new AjaxLink<Void>("setAsDefault") {
          ...
          public void onClick(AjaxRequestTarget target) {
            ...
            templateService.editTemplate(
                temp.getProjectId(), item.getModelObject().getId(), null, Model.of(temp));
            List<Template> tempModel =
  ...
  public void onEvent(IEvent<?> event) {
    ...
    if (payload instanceof TemplateFilter) {
      ...
      RequestCycle.get()
          ...
          .ifPresent(target -> target.add(this, this.getMarkupId()));
    }
  }
}
