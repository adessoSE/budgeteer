...
public class EditBudgetPage extends DialogPageWithBacklink {
  ...
  public EditBudgetPage(
      ...
      boolean isEditingNewBudget) {
    super(parameters, backlinkPage, backlinkParameters);
    isEditing = true;
    EditBudgetData budgetData = service.loadBudgetToEdit(getBudgetId());
    Form<EditBudgetData> form =
        new EditBudgetForm("form", Model.of(budgetData), isEditingNewBudget);
    addComponents(form);
  }
  ...
  public EditBudgetPage(PageParameters parameters) {
    ...
    if (getBudgetId() == 0) {
      ...
      isEditing = true;
      EditBudgetData budgetData = service.loadBudgetToEdit(getBudgetId());
      Form<EditBudgetData> form = new EditBudgetForm("form", Model.of(budgetData), false);
      addComponents(form);
    }
  }
