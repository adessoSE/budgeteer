...
public class EditUserForm extends Form<EditUserData> {
  ...
  private void addComponents() {
    ...
    RequiredTextField<String> usernameRequiredTextField =
        new RequiredTextField<>("username", model(from(getModelObject().getName())));
    EmailTextField mailTextField;
    if (getModelObject().getMail() == null) mailTextField = new EmailTextField("mail");
    else mailTextField = new EmailTextField("mail", model(from(getModelObject().getMail())));
        ...
        new Button("submitButton") {
          ...
          public void onSubmit() {
            ...
            try {
              ...
              EditUserForm.this.getModelObject().setPassword(currentPasswordTextField.getInput());
              EditUserForm.this.getModelObject().setNewPassword(newPasswordTextField.getInput());

              // If the user has changed his mail address, this will be displayed to him.
              if (!userService.saveUser(EditUserForm.this.getModelObject())) {
