...
public class DifferenceTable extends GenericPanel<List<ContractStatisticBean>> {
	...
	protected void onInitialize() {
		...
		table.add(
				new ListView<ContractStatisticBean>("row", getModelObject()) {
					@Override
					protected void populateItem(ListItem<ContractStatisticBean> item) {
						WebMarkupContainer month = new WebMarkupContainer("month");
						String monthNumber =
								item.getModelObject().getMonth() < 10
										? "0" + item.getModelObject().getMonth()
										: "" + item.getModelObject().getMonth();
						...
						month.add(
								new Label(
										...
										PropertyLoader.getProperty(
												BasePage.class,
												("monthRenderer.name." + item.getModelObject().getMonth()))));
						item.add(month);
						item.add(
								new Label(
										"invoiced",
										Model.of(
												MoneyUtil.toDouble(
														MoneyUtil.createMoneyFromCents(
																item.getModelObject().getInvoicedBudget()),
						...
						item.add(
								new Label(
										...
										Model.of(
												MoneyUtil.toDouble(
														MoneyUtil.createMoneyFromCents(item.getModelObject().getSpentBudget()),
														BudgeteerSession.get().getSelectedBudgetUnit()))));
						item.add(
								new Label(
										"difference",
										Model.of(
												MoneyUtil.toDouble(
														MoneyUtil.createMoneyFromCents(item.getModelObject().getDifference()),
														BudgeteerSession.get().getSelectedBudgetUnit()))));
						item.add(
								new ProgressBar(
		...
		for (ContractStatisticBean b : getModelObject()) {
			...
			progressInPercent =
					Math.max(
							progressInPercent, b.getProgressInPercent() == null ? 0 : b.getProgressInPercent());
		}
		table.add(
				new Label(
						"sumInvoiced",
						Model.of(
								MoneyUtil.toDouble(
										MoneyUtil.createMoneyFromCents(sumInvoiced),
										BudgeteerSession.get().getSelectedBudgetUnit()))));
		table.add(
				new Label(
						"sumSpend",
						Model.of(
								MoneyUtil.toDouble(
										MoneyUtil.createMoneyFromCents(sumSpend),
										BudgeteerSession.get().getSelectedBudgetUnit()))));
		table.add(
				new Label(
						"sumDifference",
						Model.of(
								MoneyUtil.toDouble(
										MoneyUtil.createMoneyFromCents(sumDifference),
										BudgeteerSession.get().getSelectedBudgetUnit()))));
