	...
	public BurnedBudgetChartConfiguration(BurnedBudgetChartModel model) {
		...
		List<String> labelList =
				ChartUtils.getWeekLabels(
						model.getNumberOfWeeks(),
						PropertyLoader.getProperty(BurnedBudgetChart.class, "chart.weekLabelFormat"));
		...
		Dataset dataset =
				new Dataset()
						...
						.setData(
								DoubleValue.of(
										MoneyUtil.toDouble(
												model.getObject(), BudgeteerSession.get().getSelectedBudgetUnit())))
						.setLabel(PropertyLoader.getProperty(BurnedBudgetChart.class, "chart.seriesName"));

		setData(new Data().setLabels(TextLabel.of(labelList)).setDatasets(Arrays.asList(dataset)));
