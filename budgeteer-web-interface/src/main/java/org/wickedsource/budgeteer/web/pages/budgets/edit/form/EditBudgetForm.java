...
public class EditBudgetForm extends Form<EditBudgetData> {
  ...
  private void addComponents() {
    TagsTextField tagsField =
        new TagsTextField(
            "tagsInput", of(getModel(), EditBudgetData::getTags, EditBudgetData::setTags));
          ...
          protected void onEvent(AjaxRequestTarget target) {
            if (getModelObject().getTags().size() > 0) {
              BudgeteerSession.get()
                  .getBudgetFilter()
                  .getSelectedTags()
                  .remove(getModelObject().getTags().remove(0));
            }
          }
        });
    ...
    add(
        new RequiredTextField<>(
            "name", of(getModel(), EditBudgetData::getTitle, EditBudgetData::setTitle)));
    add(
        new TextField<>(
            "description",
            of(getModel(), EditBudgetData::getDescription, EditBudgetData::setDescription)));
    add(
        new RequiredTextField<>(
            "importKey",
            of(getModel(), EditBudgetData::getImportKey, EditBudgetData::setImportKey)));
    MoneyTextField totalField =
        new MoneyTextField(
            "total", of(getModel(), EditBudgetData::getTotal, EditBudgetData::setTotal));
    totalField.setRequired(true);
    add(totalField);
    MoneyTextField limitField =
        new MoneyTextField(
            "limit", of(getModel(), EditBudgetData::getLimit, EditBudgetData::setLimit));
    add(limitField);
    DropDownChoice<ContractBaseData> contractDropDown =
        new DropDownChoice<>(
            "contract",
            of(getModel(), EditBudgetData::getContract, EditBudgetData::setContract),
    ...
    if (isEditing) {
      ...
      submitButtonLabel =
          new Label("submitButtonLabel", new ResourceModel("button.save.createmode"));
    }
  ...
  private ListView<String> createTagsList(
    ...
    return new ListView<String>(id, model) {
      ...
      protected void populateItem(final ListItem<String> item) {
        Label label = new Label("tag", item.getModel());
  ...
  protected void onSubmit() {
    try {
      if (!isEditing) {
        ...
        setResponsePage(
            new EditBudgetPage(
                EditBudgetPage.createParameters(newID),
