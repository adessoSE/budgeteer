...
public class BudgetNotesForm extends Form<EditBudgetData> {
  ...
  /**
   * Add all components to the view. The editor with settings and a CustomFeedbackPanel are added.
   */
  private void addComponents() {
    TinyMCESettings settings =
        new TinyMCESettings(TinyMCESettings.Theme.modern, TinyMCESettings.Language.en_GB);

    // The names of the plugins are stored in a string array and then added to the settings in a
    // for-each loop.
    String plugins =
    ...
    for (String name : pluginNames) settings.addPlugins(name);
    ...
    settings.addToolbar(
        new Toolbar(
            "toolbar3", "table link unlink image | hr pagebreak | visualchars visualblocks"));

    TextArea<String> textArea =
        new TextArea<>("editor", of(getModel(), EditBudgetData::getNote, EditBudgetData::setNote));
