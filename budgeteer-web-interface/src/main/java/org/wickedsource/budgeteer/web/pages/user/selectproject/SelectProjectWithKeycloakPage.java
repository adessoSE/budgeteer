...
public class SelectProjectWithKeycloakPage extends DialogPage {
	...
	private Form<String> createNewProjectForm(String id) {
		Form<String> form =
				new Form<String>(id, new Model<String>("")) {
					@Override
					protected void onSubmit() {
						ProjectBaseData project =
	...
	private Form<ProjectBaseData> createChooseProjectForm(String id) {
		LoadableDetachableModel<ProjectBaseData> defaultProjectModel =
				new LoadableDetachableModel<ProjectBaseData>() {

					@Override
					protected ProjectBaseData load() {
						return projectService.getDefaultProjectForUser(
								BudgeteerSession.get().getLoggedInUser().getId());
					}
				};
		final Form<ProjectBaseData> form =
				new Form<ProjectBaseData>("chooseProjectForm", defaultProjectModel);

		DropDownChoice<ProjectBaseData> choice =
				new DropDownChoice<ProjectBaseData>(
						"projectChoice",
						form.getModel(),
						new ProjectsForUserModel(BudgeteerSession.get().getLoggedInUser().getId()),
		...
		AjaxSubmitLink markProjectAsDefault =
					...
					protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
						try {
							projectService.setDefaultProject(
									BudgeteerSession.get().getLoggedInUser().getId(),
									((ProjectBaseData) form.getModelObject()).getId());
