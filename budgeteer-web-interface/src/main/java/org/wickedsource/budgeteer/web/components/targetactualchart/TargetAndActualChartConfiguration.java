	...
	public TargetAndActualChartConfiguration(IModel<TargetAndActual> model, Mode mode) {
		...
		if (model.getObject() != null) {
			...
			for (int i = 0; i < model.getObject().getActualSeries().size(); i++) {
				...
				Dataset newDataset =
						new Dataset()
								.setLabel(series.getName())
								.setData(
										DoubleValue.of(
												MoneyUtil.toDouble(
														series.getValues(), BudgeteerSession.get().getSelectedBudgetUnit())))
			...
			Dataset planDataset =
					new Dataset()
							...
							.setData(
									DoubleValue.of(
											MoneyUtil.toDouble(
													model.getObject().getTargetSeries().getValues(),
		...
		setOptions(
				new Options()
						...
						.setResponsive(true)
						.setTooltips(new Tooltips().setIntersect(true).setMode(TooltipMode.INDEX))
						...
						.setScales(
								new Scales()
										.setXAxes(
												new AxesScale()
														.setStacked(true)
														.setGridLines(new GridLines().setDisplay(false).setDrawBorder(false)))
										.setYAxes(
												new AxesScale()
														.setStacked(true)
														.setGridLines(new GridLines().setDrawBorder(false)))));
	}
}
