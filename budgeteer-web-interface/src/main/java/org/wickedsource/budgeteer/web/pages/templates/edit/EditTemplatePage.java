...
public class EditTemplatePage extends DialogPageWithBacklink {
	...
	public EditTemplatePage(
			Class<? extends WebPage> backlinkPage, PageParameters backlinkParameters, long templateID) {
								...
								if (model(from(templateFormInputDto)).getObject().getName() != null
									...
									templateId =
											service.editTemplate(
													...
													templateId,
													file,
													model(from(templateFormInputDto)));
									success(getString("message.success"));
									updateTemplateID(templateId);
								}
							} else if (model(from(templateFormInputDto)).getObject().getName() != null
									&& model(from(templateFormInputDto)).getObject().getDescription() != null) {
								templateId =
										service.editTemplate(
												BudgeteerSession.get().getProjectId(),
			...
			public void onClick() {
				...
				ResourceStreamRequestHandler handler =
						new ResourceStreamRequestHandler(
								streamWriter, service.getById(templateID).getName() + ".xlsx");
