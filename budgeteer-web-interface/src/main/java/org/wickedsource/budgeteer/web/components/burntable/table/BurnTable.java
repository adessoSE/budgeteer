...
public class BurnTable extends Panel {
  ...
  public BurnTable(String id, FilteredRecordsModel model, boolean dailyRateIsEditable) {
    ...
    IModel<Money> totalSum =
        () ->
            model.getObject().stream()
                .map(WorkRecord::getBudgetBurned)
                .reduce(MoneyUtil.ZERO, Money::plus);
  ...
  private void createTableInfoLabel() {
    Label pageLabel =
        new Label(
            "pageLabel",
            "Showing "
                + Long.toString(rows.getFirstItemOffset() + 1)
                + " to "
                + Long.toString(rows.getFirstItemOffset() + rows.getItemsPerPage())
                + " entries from total "
                + getRows().getItemCount()) {
          @Override
            ...
            if (rows.getPageCount() == 1L) {
              ...
              this.setDefaultModelObject(
                  "Showing "
                      ...
                      + " to "
                      + Long.toString(
                          rows.getFirstItemOffset()
                              + (rows.getItemCount() % rows.getItemsPerPage()))
                      + " entries from total "
                      + getRows().getItemCount());
            } else {
              this.setDefaultModelObject(
  ...
  private void createItemsPerPageInput() {
    ...
    form.add(
        new NumberTextField<>("itemsPerPage", recordsPerPageModel)
            .setMinimum(1L)
            .setMaximum(rows.getItemCount()));
  ...
  /**
   * This mimics the exact paging functionality of DataTables.js. I could not use the standard
   * DataTables paging, because that plugin has no idea the table even has more pages to it (as
   * Wicket doesn't load eveything at once) - it just wants to have all the data at page creation
   * time.
   */
  private void createPageNavigation() {
    RepeatingView pageNavButtons =
        new RepeatingView("pageNavButtons") {
          @Override
          protected void onBeforeRender() {
            super.onBeforeRender();
            this.removeAll();
            ...
            if (rows.getPageCount() < 5) {
              ...
              if (rows.getCurrentPage() < 3) {
              ...
              } else if (rows.getCurrentPage()
                  > rows.getPageCount() - 4) { // If we are on the last 3 Pages
                ...
                add(
                    createNumberedNavButton(newChildId(), rows.getCurrentPage())
    ...
    previousPageButton =
        new AjaxLink<Void>("previousPage") {
          @Override
  ...
  private DataView<WorkRecord> createList(
      final FilteredRecordsModel model, final WebMarkupContainer table) {
    return new DataView<WorkRecord>(
        "recordList",
        new ListDataProvider<WorkRecord>(model.getObject()) {
          @Override
          ...
          final EditableMoneyField editableMoneyField =
              new EditableMoneyField(
                ...
                protected void save(AjaxRequestTarget target, Form<Money> form) {
                  ...
                  long elementIndex = 0;
                  int i = 0;
                  Iterator<? extends WorkRecord> iterator =
                      rows.getDataProvider().iterator(0, rows.getItemCount());
