...
public class BurnTable extends Panel {
	...
	private ListView<WorkRecord> createList(
			String id, final IModel<List<WorkRecord>> model, final WebMarkupContainer table) {
		return new ListView<WorkRecord>(id, model) {
			...
			protected ListItem<WorkRecord> newItem(int index, IModel<WorkRecord> itemModel) {
				...
				return super.newItem(
						index, new ClassAwareWrappingModel<WorkRecord>(itemModel, WorkRecord.class));
			}
		};
	}
