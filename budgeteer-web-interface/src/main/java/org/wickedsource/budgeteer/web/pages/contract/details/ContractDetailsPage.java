...
public class ContractDetailsPage extends BasePage {
	...
	public ContractDetailsPage(PageParameters parameters) {
		...
		add(
				new ContractDetailChart(
						"comparisonChart", new ContractDetailChartModel(getParameterId(), numberOfMonths)));
		add(
				new DifferenceTable(
						"differenceTable",
						new DifferenceTableModel(getParameterId(), contractModel.getObject().getStartDate())));
		...
		add(
				new Link("showInvoiceLink") {
					@Override
					public void onClick() {
						WebPage page =
								new InvoiceOverviewPage(InvoiceOverviewPage.createParameters(getParameterId())) {
		...
		add(
				new Link("showContractLink") {
					...
					public void onClick() {
						WebPage page =
								new BudgetForContractOverviewPage(
										BudgetForContractOverviewPage.createParameters(getParameterId())) {

									@Override
									protected BreadcrumbsModel getBreadcrumbsModel() {
										BreadcrumbsModel m = ContractDetailsPage.this.getBreadcrumbsModel();
										m.addBreadcrumb(
												BudgetForContractOverviewPage.class,
												BudgetForContractOverviewPage.createParameters(getParameterId()));
										return m;
									}
								};
