...
public interface ManualRecordRepository extends CrudRepository<ManualRecordEntity, Long>, JpaSpecificationExecutor<ManualRecordEntity> {
    ...
    @Query("select new org.wickedsource.budgeteer.persistence.record.MonthlyAggregatedRecordWithTaxBean(r.year, r.month, coalesce(sum(r.moneyAmount),0), r.budget.contract.taxRate ) from ManualRecordEntity r where r.budget.id=:budgetId group by r.year, r.month, r.budget.contract.taxRate order by r.year, r.month")
    List<MonthlyAggregatedRecordWithTaxBean> aggregateByMonthAndBudgetWithTax(@Param("budgetId") long budgetId);

    @Query("select new org.wickedsource.budgeteer.persistence.record.MonthlyAggregatedRecordWithTaxBean(r.year, r.month, coalesce(sum(r.moneyAmount),0), r.budget.contract.taxRate) from ManualRecordEntity r join r.budget b where b.project.id=:projectId group by r.year, r.month, r.budget.contract.taxRate order by r.year, r.month")
