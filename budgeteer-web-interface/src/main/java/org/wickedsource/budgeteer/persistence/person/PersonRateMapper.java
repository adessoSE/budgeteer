...
public class PersonRateMapper {
    public PersonRate mapToDomain(DailyRateEntity dailyRateEntity) {
        return new PersonRate(dailyRateEntity.getRate(), dailyRateEntity.getBudget().getId(),
                new DateRange(new Date(dailyRateEntity.getDateStart().getTime()).toLocalDate(),
    ...
    public List<PersonRate> mapToDomain(List<DailyRateEntity> dailyRateEntities) {
        return dailyRateEntities.stream()
