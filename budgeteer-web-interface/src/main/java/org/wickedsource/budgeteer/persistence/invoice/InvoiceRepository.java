...
public interface InvoiceRepository extends CrudRepository<InvoiceEntity, Long> {
	...
	@Modifying
	@Query(
			"delete from InvoiceEntity ie where ie.id in (select id from InvoiceEntity where contract.project.id = :projectId)")
	void deleteByProjectId(@Param(value = "projectId") long projectId);

	@Modifying
	@Query(
			"delete from ContractInvoiceField where id in (select id from ContractInvoiceField where contract.project.id = :projectId)")
	void deleteContractInvoiceFieldByProject(@Param("projectId") long projectId);

	@Modifying
	@Query(
			"delete from InvoiceFieldEntity cif where cif.id in (select id from InvoiceFieldEntity where field.contract.project.id = :projectId )")
	...
	@Modifying
	@Query(
			"delete from InvoiceFieldEntity cif where cif.id in (select id from InvoiceFieldEntity where field.contract.id = :contractId )")
