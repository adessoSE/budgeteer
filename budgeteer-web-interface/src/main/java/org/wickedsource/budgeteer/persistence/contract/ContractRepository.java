...
public interface ContractRepository extends CrudRepository<ContractEntity, Long> {
	...
	@Query(
			"select cif from ContractInvoiceField cif where cif.contract.id = :contractId AND cif.fieldName = :fieldName")
	ContractInvoiceField findInvoiceFieldByName(
			@Param("contractId") long contractID, @Param("fieldName") String fieldName);

	/**
	 * returns a ContractStatisticBean for a given contract till the given month and year. returns the
	 * remaining budget of the contract, the spend budget in budgeteer and the invoiced budget until
	 * the given date
	 */
	@Query(
			"select new org.wickedsource.budgeteer.persistence.contract.ContractStatisticBean(:year+0,"
	...
	ContractStatisticBean getContractStatisticAggregatedByMonthAndYear(
	...
	/**
	 * returns a ContractStatisticBean for a given contract till the given month and year. returns the
	 * remaining budget of the contract, the spend budget in budgeteer and the invoiced budget for the
	 * given month
	 */
	@Query(
			"select new org.wickedsource.budgeteer.persistence.contract.ContractStatisticBean(:year+0,"
					...
					+ ",:month +0"
					+ ") from ContractEntity c where c.id = :contractId")
	ContractStatisticBean getContractStatisticByMonthAndYear(
			@Param("contractId") Long contractId,
			@Param("month") Integer month,
			@Param("year") Integer year);
