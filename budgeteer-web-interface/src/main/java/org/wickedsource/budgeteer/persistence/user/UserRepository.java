...
public interface UserRepository extends CrudRepository<UserEntity, Long> {
	...
	public UserEntity findByNameAndPassword(
	...
	@Query(
			"select u from UserEntity u where u.id not in (select u2.id from UserEntity u2 join u2.authorizedProjects p where p.id = :projectId)")
