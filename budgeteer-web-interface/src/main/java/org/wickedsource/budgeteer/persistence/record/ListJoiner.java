...
public class ListJoiner {
   ...
   */
  public static List<WeeklyAggregatedRecordWithTaxBean> joinPlanBeanHours(
    ...
    for (WeeklyAggregatedRecordWithTaxBean bean : beans) {
      WeeklyAggregatedRecordWithTaxBean record =
          planMap.get(
              String.format(
                  "%d%d%d%s", bean.getYear(), bean.getMonth(), bean.getWeek(), bean.getTaxRate()));
      if (record == null) {
        planMap.put(
            String.format(
                "%d%d%d%s", bean.getYear(), bean.getMonth(), bean.getWeek(), bean.getTaxRate()),
   ...
   */
  public static List<WeeklyAggregatedRecordWithTitleAndTaxBean> joinWorkBeanHours(
      List<WeeklyAggregatedRecordWithTitleAndTaxBean> beans) {
    ...
    for (WeeklyAggregatedRecordWithTitleAndTaxBean bean : beans) {
      WeeklyAggregatedRecordWithTitleAndTaxBean record =
          workMap.get(
              String.format(
                  "%d%d%d%s%s",
                  bean.getYear(),
  ...
  /**
   * Joins a list with weekly plan beans and a list with weekly work beans to a list of
   * PlanAndWorkRecords
   *
   ...
   */
  public static List<PlanAndWorkRecord> joinWeeklyToPlanAndWorkRecords(
    ...
    for (WeeklyAggregatedRecordWithTaxBean bean : planList) {
      ...
      if (record == null) {
        planAndWorkMap.put(
            String.format(
                "%d%d%d%s", bean.getYear(), bean.getMonth(), bean.getWeek(), bean.getTaxRate()),
            new PlanAndWorkRecord(bean));
      } else {
        record.setValueInCentsPlanned(record.getValueInCentsPlanned() + bean.getValueInCents());
        record.setValueInCentsPlanned_gross(
            MoneyUtil.getCentsWithTaxes(record.getValueInCentsPlanned(), record.getTaxRate()));
    ...
    for (WeeklyAggregatedRecordWithTitleAndTaxBean bean : workList) {
      ...
      if (record == null) {
        ...
        record.setValueInCentsBurned_gross(
            MoneyUtil.getCentsWithTaxes(record.getValueInCentsBurned(), record.getTaxRate()));
  ...
  /**
   * Joins a list with monthly plan beans and a list with monthly work beans to a list of
   * PlanAndWorkRecords
   *
   ...
   * @return joined beans as a list of PlanAndWorkRecords
   */
  public static List<PlanAndWorkRecord> joinMonthlyToPlanAndWorkRecords(
      List<MonthlyAggregatedRecordWithTaxBean> planList,
      List<MonthlyAggregatedRecordWithTaxBean> workList) {
    HashMap<String, PlanAndWorkRecord> planAndWorkMap = new HashMap<>();
    ...
    for (MonthlyAggregatedRecordWithTaxBean planBean : planList) {
      PlanAndWorkRecord record =
          planAndWorkMap.get(
              String.format(
                  "%s%d%d", planBean.getTaxRate(), planBean.getYear(), planBean.getMonth()));
      if (record == null) {
        planAndWorkMap.put(
            String.format("%s%d%d", planBean.getTaxRate(), planBean.getYear(), planBean.getMonth()),
            new PlanAndWorkRecord(planBean, true));
      } else {
    ...
    for (MonthlyAggregatedRecordWithTaxBean workBean : workList) {
      PlanAndWorkRecord record =
          planAndWorkMap.get(
              String.format(
                  "%s%d%d", workBean.getTaxRate(), workBean.getYear(), workBean.getMonth()));
      if (record == null) {
        planAndWorkMap.put(
            String.format("%s%d%d", workBean.getTaxRate(), workBean.getYear(), workBean.getMonth()),
            new PlanAndWorkRecord(workBean, false));
      } else {
