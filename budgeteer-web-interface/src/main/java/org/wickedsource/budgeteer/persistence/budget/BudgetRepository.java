...
public interface BudgetRepository extends CrudRepository<BudgetEntity, Long> {
    ...
    @Query("select new org.wickedsource.budgeteer.persistence.budget.LimitReachedBean(b.id, b.name) from BudgetEntity b where b.limit <= :spent and b.limit != 0L and b.id=:budgetId order by b.name")
