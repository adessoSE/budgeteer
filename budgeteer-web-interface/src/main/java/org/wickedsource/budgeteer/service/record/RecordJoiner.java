...
public class RecordJoiner {
  ...
  /**
   * Joins the given work records and the given plan records since it is not possible to join them
   * in the database with JPA.
   */
  public List<AggregatedRecord> joinWeekly(
      List<WeeklyAggregatedRecordBean> workRecords, List<WeeklyAggregatedRecordBean> planRecords) {
   ...
   */
  public List<AggregatedRecord> joinWeeklyByMonthFraction(
    ...
    List<WeeklyAggregatedRecordWithTitleAndTaxBean> workList =
        ListJoiner.joinWorkBeanHours(workRecords);
  ...
  /**
   * Goes through a list of records by time periods and sums up the money of all records for each
   * period.
   *
  ...
  private List<AggregatedRecord> sumRecordsForEachPeriod(List<AggregatedRecord> records) {
    ...
    while (i < records.size() - 1) {
      ...
      if (current.getAggregationPeriodStart().equals(next.getAggregationPeriodStart())
          && current.getAggregationPeriodEnd().equals(next.getAggregationPeriodEnd())) {
        ...
        sum.setBudgetPlanned_net(
            MoneyUtil.sumMoney(current.getBudgetPlanned_net(), next.getBudgetPlanned_net()));
        sum.setBudgetPlanned_gross(
            MoneyUtil.sumMoney(current.getBudgetPlanned_gross(), next.getBudgetPlanned_gross()));
        sum.setBudgetBurned_net(
            MoneyUtil.sumMoney(current.getBudgetBurned_net(), next.getBudgetBurned_net()));
        sum.setBudgetBurned_gross(
            MoneyUtil.sumMoney(current.getBudgetBurned_gross(), next.getBudgetBurned_gross()));
  ...
  public List<AggregatedRecord> joinMonthly(
  ...
  public List<AggregatedRecord> joinMonthlyWithTax(
      List<MonthlyAggregatedRecordWithTaxBean> workRecords,
      List<MonthlyAggregatedRecordWithTaxBean> planRecords) {
    List<PlanAndWorkRecord> planAndWorkRecordList =
        ListJoiner.joinMonthlyToPlanAndWorkRecords(planRecords, workRecords);
  ...
  class RecordMap extends HashMap<String, AggregatedRecord> {
    ...
    public AggregatedRecord getOrCreateRecord(WeeklyAggregatedRecordBean recordBean) {
      ...
      if (record == null) {
        ...
        record.setAggregationPeriodTitle(
            String.format("Week %d-%d", recordBean.getYear(), recordBean.getWeek()));
    ...
    public AggregatedRecord getOrCreateRecord(MonthlyAggregatedRecordBean recordBean) {
      ...
      if (record == null) {
        ...
        record.setAggregationPeriodEnd(c.getTime());
        record.setAggregationPeriodTitle(
            String.format("Month %d-%02d", recordBean.getYear(), recordBean.getMonth() + 1));
        put(getKey(recordBean), record);
      }
      return record;
    }
