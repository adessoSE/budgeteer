...
import de.adesso.budgeteer.persistence.budget.BudgetTagEntity;
...
public class BudgetService {
  ...
  @Autowired private ContractDataMapper contractDataMapper;
  ...
  private List<BudgetEntity> loadBudgetEntities(long projectId, BudgetTagFilter filter) {
    ...
    if (filter.getSelectedTags().isEmpty()) {
      ...
      budgets =
          new ArrayList<>(
              new TreeSet<>(
                  budgetRepository.findByAtLeastOneTag(projectId, filter.getSelectedTags())));
    }
    return budgets;
  }
  ...
  public List<BudgetDetailData> loadBudgetsDetailData(long projectId, BudgetTagFilter filter) {
    ...
    for (BudgetEntity entity : budgets) {
      // TODO: 4 additional database queries per loop! These can yet be optimized to 4 queries
      // total!
   ...
   * @return list of budgets that match the filter.
   */
  public List<BudgetDetailData> loadBudgetsDetailData(
      long projectId, BudgetTagFilter filter, Long remainingFilter) {
      ...
      if (!BudgeteerSession.get().isTaxEnabled()) {
        if (e.getRemaining()
            .isGreaterThan(
                () ->
                    BigMoney.of(
                        e.getRemaining().getCurrencyUnit(), new BigDecimal(remainingFilter)))) {
          result.add(e);
        }
      } else {
        if (e.getRemaining_gross()
            .isGreaterThan(
                () ->
                    BigMoney.of(
                        e.getRemaining_gross().getCurrencyUnit(),
                        new BigDecimal(remainingFilter)))) {
          result.add(e);
        }
      }
    }
  ...
  public EditBudgetData loadBudgetToEdit(long budgetId) {
    BudgetEntity budget =
        budgetRepository
            .findById(budgetId)
            .orElseThrow(() -> new UnknownEntityException(BudgetEntity.class, budgetId));
  ...
  public long saveBudget(EditBudgetData data) {
    ...
    if (data.getId() != 0) {
      ...
      ProjectEntity project =
          projectRepository.findById(data.getProjectId()).orElseThrow(RuntimeException::new);
      budget.setProject(project);
    }

    boolean duplicateImportKey =
        !Objects.equals(budget.getImportKey(), data.getImportKey())
            && budgetRepository.existsByImportKeyAndProjectId(
                data.getImportKey(), data.getProjectId());
    boolean duplicateName =
        !Objects.equals(budget.getName(), data.getTitle())
            && budgetRepository.existsByNameAndProjectId(data.getTitle(), data.getProjectId());
    ...
    if (data.getContract() == null) {
      ...
      ContractEntity contractEntity =
          contractRepository
              .findById(data.getContract().getContractId())
  ...
  /**
   * Returns the units in which the user can have his budget values displayed. One unit is active,
  ...
  public List<OptionGroup<BudgetBaseData>> getPossibleBudgetDataForPersonAndProject(
      long projectId, long personId) {

    List<BudgetBaseData> allBudgets =
  ...
  private List<BudgetBaseData> loadBudgetBaseDataForPerson(long personId) {
    List<BudgetEntity> budgets =
        new ArrayList<>(new TreeSet<>(budgetRepository.findByPersonId(personId)));
    return budgetBaseDataMapper.map(budgets);
  }
