...
public class PersonService {
  ...
  @Autowired private PersonBaseDataMapper personBaseDataMapper;

  @Autowired private PersonDetailDataMapper personDetailDataMapper;
   ...
   * @return list of PersonBaseData objects for all people the given user can make use of to manage
   *     budgets.
   */
  ...
  public List<String> savePersonWithRates(PersonWithRates person) {
    PersonEntity personEntity =
        personRepository.findById(person.getPersonId()).orElseThrow(RuntimeException::new);
    ...
    for (PersonRate rate : person.getRates()) {
      List<String> errorList =
          this.updateDailyRates(rate.getBudget(), person, rate.getDateRange(), rate.getRate());
  ...
  public List<String> getOverlapWithManuallyEditedRecords(PersonWithRates person, long projectId) {
    ...
    for (PersonRate rate : person.getRates()) {
      for (WorkRecordEntity e :
          workRecordRepository.findManuallyEditedEntries(
              projectId, rate.getDateRange().getStartDate(), rate.getDateRange().getEndDate())) {

        // Warn about the editing of a rate only if a work record in this range has been edited
        // manually and the amount is different
        if (DateUtil.isDateInDateRange(e.getDate(), rate.getDateRange())
          ...
          warnings.add(
              "A work record in the range "
                  + rate.getDateRange().toString()
                  + " (Exact Date and Amount: "
                  + e.getDate()
                  + ", "
                  + e.getDailyRate().toString()
                  + ") for budget \""
                  + rate.getBudget().getName()
  ...
  public void removeDailyRateFromPerson(PersonWithRates personWithRates, PersonRate rate) {
    this.updateDailyRates(
        rate.getBudget(), personWithRates, rate.getDateRange(), Money.zero(CurrencyUnit.EUR));
  }

  private List<String> updateDailyRates(
      BudgetBaseData budget, PersonWithRates person, DateRange dateRange, Money dailyRate) {
