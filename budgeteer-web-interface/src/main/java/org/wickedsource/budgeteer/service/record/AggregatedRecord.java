  ...
  public AggregatedRecord(PlanAndWorkRecord planAndWorkRecord) {
    ...
    if (planAndWorkRecord.getWeek() == -1) {
      ...
      aggregationPeriodTitle =
          String.format(
              "Month %d-%02d", planAndWorkRecord.getYear(), planAndWorkRecord.getMonth() + 1);
    } else {
      c.set(Calendar.WEEK_OF_YEAR, planAndWorkRecord.getWeek());
      aggregationPeriodStart = c.getTime();
      c.add(Calendar.DAY_OF_YEAR, 6);
      aggregationPeriodTitle =
          String.format("Week %d-%d", planAndWorkRecord.getYear(), planAndWorkRecord.getWeek());
    }
    ...
    budgetPlanned_gross =
        MoneyUtil.createMoneyFromCents(planAndWorkRecord.getValueInCentsPlanned_gross());
    budgetBurned_net = MoneyUtil.createMoneyFromCents(planAndWorkRecord.getValueInCentsBurned());
    budgetBurned_gross =
        MoneyUtil.createMoneyFromCents(planAndWorkRecord.getValueInCentsBurned_gross());
  }
