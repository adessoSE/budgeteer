...
public class WorkRecordQueries {
  ...
  public static Specification<WorkRecordEntity> findByFilter(WorkRecordFilter filter) {
    return (record, query, builder) -> {
      Predicate projectPredicate =
          builder.equal(record.get("budget").get("project").get("id"), filter.getProjectId());
      return builder.and(
          projectPredicate,
  ...
  private static Predicate buildStartDatePredicate(
      WorkRecordFilter filter, Root<WorkRecordEntity> record, CriteriaBuilder builder) {
  ...
  private static Predicate buildEndDatePredicate(
      WorkRecordFilter filter, Root<WorkRecordEntity> record, CriteriaBuilder builder) {
  ...
  private static Predicate buildBudgetPredicate(
      WorkRecordFilter filter, Root<WorkRecordEntity> record, CriteriaBuilder builder) {
    if (filter.getBudgetList().isEmpty()) {
      return builder.and();
    }
    return builder.or(
        filter.getBudgetList().stream()
            .map(budget -> builder.equal(record.get("budget").get("id"), budget.getId()))
            .toArray(Predicate[]::new));
  }

  private static Predicate buildPersonPredicate(
      WorkRecordFilter filter, Root<WorkRecordEntity> record, CriteriaBuilder builder) {
    if (filter.getPersonList().isEmpty()) {
      return builder.and();
    }
    return builder.or(
        filter.getPersonList().stream()
