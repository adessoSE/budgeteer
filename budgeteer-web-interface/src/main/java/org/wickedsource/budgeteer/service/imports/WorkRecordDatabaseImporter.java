...
public class WorkRecordDatabaseImporter extends RecordDatabaseImporter {
  ...
  @Getter @Setter private Date earliestRecordDate = new Date(Long.MAX_VALUE);

  @Getter @Setter private Date latestRecordDate = new Date(1l);
  ...
  public void importRecords(List<ImportedWorkRecord> records) {
    ...
    for (ImportedWorkRecord record : records) {
      ...
      if ((project.getProjectEnd() != null && record.getDate().after(project.getProjectEnd()))
          || (project.getProjectStart() != null
              && record.getDate().before(project.getProjectStart()))) {
    ...
    // If all records haven been skipped the startDate of the import is new Date(Long.MAX_VALUE) and
    // the EndDate is null.
   ...
   * This methods seeks and deletes these entries. If there is both a manually edited and a newly
   * imported entry, the imported one will be deleted. If there are any manually edited entries
   * without corresponding imported entries, the manually edited entry will be seen as obsolete and
   * be deleted too.
   *
   * @return a list of entries that were deleted during checking for duplicates. See Issue #72
   */
  public List<List<String>> findAndRemoveManuallyEditedEntries() {
    ...
    headline.add("Miscellaneous");
    deletedRecordList.add(headline);
    // Find duplicate Records, where the dailyRate was edited manually
    List<WorkRecordEntity> manuallyEditedEntries =
        workRecordRepository.findManuallyEditedEntries(
            super.getProjectId(), getEarliestRecordDate(), getLatestRecordDate());
    for (int i = manuallyEditedEntries.size() - 1; i >= 0; i--) {
      WorkRecordEntity editedEntry = manuallyEditedEntries.get(i);
      List<WorkRecordEntity> duplicateEntryList =
          workRecordRepository.findDuplicateEntries(
              editedEntry.getBudget(),
              editedEntry.getPerson(),
              editedEntry.getDate(),
              editedEntry.getMinutes());
      if (!duplicateEntryList.isEmpty()) {
        deletedRecordList.add(
            getRecordAsString(
                duplicateEntryList.get(0),
                "There was a manually edited entry associated with this one already in the database"));
        workRecordRepository.delete(duplicateEntryList.get(0));
        // Update the import-reference of the the manually edited record so that it belongs to the
        // current import
    ...
    // if there are still any manually edited records in the list, there weren't any associated
    // entries in the current import
    ...
    for (WorkRecordEntity leftRecord : manuallyEditedEntries) {
      ...
      deletedRecordList.add(
          getRecordAsString(
              leftRecord,
              "The record was edited manually in the application but didn't has a corresponding entry in the import -> deleted"));
    }
    // If just an empty row and the headline was added, return a empty list
    if (deletedRecordList.size() <= 2) return new LinkedList<List<String>>();
  ...
  private List<String> getRecordAsString(
      String budgetName, WorkRecordEntity entity, String reason) {
