...
public class TemplateService {
	...
	public List<Template> getTemplates() {
		...
		for (TemplateEntity E : templateRepository.findAll()) {
			result.add(
					new Template(E.getId(), E.getName(), E.getDescription(), E.getWb(), E.getProjectId()));
		}
		return result;
	}
	...
	public Template getById(long templateID) {
		for (TemplateEntity E : templateRepository.findAll()) {
			if (E.getId() == templateID) {
				return new Template(
						E.getId(), E.getName(), E.getDescription(), E.getWb(), E.getProjectId());
			}
		}
		return null;
	}
	 ...
	 * @return Returns the id of the new template, as it will be different (we create a new one with
	 *     the edited data and destroy the old one.
	 */
	public long editTemplate(
			long projectId,
			long templateId,
