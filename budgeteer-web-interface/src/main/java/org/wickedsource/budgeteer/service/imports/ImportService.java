...
public class ImportService implements ApplicationContextAware {
  ...
  @Autowired private ImporterRegistry importerRegistry;
  ...
  @Getter private List<List<String>> skippedRecords;
  /**
  ...
  public List<Import> loadImports(long projectId) {
    ...
    for (ImportEntity entity : imports) {
      ...
      i.setNumberOfImportedFiles(
          entity.getNumberOfImportedFiles() == null ? -1 : entity.getNumberOfImportedFiles());
   ...
   *
   * @param importer an importer that understands the format of the files represented by the input
   *     streams.
   * @param importFiles the files to be imported
   */
  @Transactional(rollbackOn = ImportException.class)
  public void doImport(long projectId, Importer importer, List<ImportFile> importFiles)
    ...
    if (importer instanceof WorkRecordsImporter) {
      ...
      WorkRecordDatabaseImporter dbImporter =
          applicationContext.getBean(
              WorkRecordDatabaseImporter.class, projectId, workRecordsImporter.getDisplayName());
