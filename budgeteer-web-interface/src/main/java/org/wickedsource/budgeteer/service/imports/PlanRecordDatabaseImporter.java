...
public class PlanRecordDatabaseImporter extends RecordDatabaseImporter {
  ...
  public void importRecords(List<ImportedPlanRecord> records, String filename) {
    ...
    // Because of Issue #70 all PlanRecords that are after the earliest of the imported ones, should
    // be deleted
    ...
    if (getImportRecord().getStartDate() == null
        || getImportRecord().getStartDate().equals(new Date(Long.MAX_VALUE))) {
  ...
  private Map<RecordKey, List<ImportedPlanRecord>> groupRecords(List<ImportedPlanRecord> records) {
    Map<RecordKey, List<ImportedPlanRecord>> result =
        new HashMap<RecordKey, List<ImportedPlanRecord>>();
    for (ImportedPlanRecord record : records) {
      RecordKey key =
          new RecordKey(record.getPersonName(), record.getBudgetName(), record.getDailyRate());
  ...
  private List<List<String>> importRecordGroup(
      RecordKey groupKey, List<ImportedPlanRecord> records, String filename) {
    ...
    if (entitiesToImport != null && !entitiesToImport.isEmpty()) {
      ...
      if (getImportRecord().getStartDate() == null
          || getImportRecord().getStartDate().after(earliestDate)) {
        getImportRecord().setStartDate(earliestDate);
      }
      if (getImportRecord().getEndDate() == null
          || getImportRecord().getEndDate().before(latestDate)) {
  ...
  private List<String> getRecordAsString(
      PlanRecordEntity recordEntity, String filename, String reason) {
