...
public class RecordService {
  ...
  @Autowired private RecordJoiner recordJoiner;

  @Autowired private WorkRecordMapper recordMapper;

  /**
   * Loads the actual budget burned by the given person and the budget planned for this person
   * aggregated by week.
   *
   * @param personId ID of the person whose target and actual records to load
   * @return one record for each week from the current week to the first week that person booked
   *     hours
   */
  public List<AggregatedRecord> getWeeklyAggregationForPerson(long personId) {
    List<WeeklyAggregatedRecordBean> planRecords =
        planRecordRepository.aggregateByWeekAndPerson(personId);
    List<WeeklyAggregatedRecordBean> workRecords =
        workRecordRepository.aggregateByWeekAndPerson(personId);
   ...
   *
   * @param personId ID of the person whose target and actual records to load
   * @return one record for each month from the current month to the first month that person booked
   *     hours
   */
  public List<AggregatedRecord> getMonthlyAggregationForPerson(long personId) {
    List<MonthlyAggregatedRecordBean> planRecords =
        planRecordRepository.aggregateByMonthAndPerson(personId);
    List<MonthlyAggregatedRecordBean> workRecords =
        workRecordRepository.aggregateByMonthAndPerson(personId);
    return recordJoiner.joinMonthly(workRecords, planRecords);
  }

  /**
   * Loads the actual budget burned in the given budget and the budget planned for all people active
   * in this budget aggregated by week.
   *
   * @param budgetId ID of the budget whose target and actual records to load
   * @return one record for each week from the current week to the first week that was booked in the
   *     given budget
   */
  public List<AggregatedRecord> getWeeklyAggregationForBudget(long budgetId) {
    List<WeeklyAggregatedRecordBean> planRecords =
        planRecordRepository.aggregateByWeekAndBudget(budgetId);
    List<WeeklyAggregatedRecordBean> workRecords =
        workRecordRepository.aggregateByWeekAndBudget(budgetId);
    return recordJoiner.joinWeekly(workRecords, planRecords);
  }

  /**
   * Loads the actual budget burned in the given budget and the budget planned for all people active
   * in this budget aggregated by week with taxes.
   *
   * @param budgetId ID of the budget whose target and actual records to load
   * @return one record for each week from the current week to the first week that was booked in the
   *     given budget
  ...
  public List<AggregatedRecord> getWeeklyAggregationForBudgetWithTax(long budgetId) {
    List<WeeklyAggregatedRecordWithTitleAndTaxBean> workRecords =
        workRecordRepository.aggregateByWeekAndPersonForBudgetWithTax(budgetId);
    List<WeeklyAggregatedRecordWithTaxBean> planRecords =
        planRecordRepository.aggregateByWeekForBudgetWithTax(budgetId);
  ...
  /**
   * Loads the actual budget burned in the given budget and the budget planned for all people active
   * in this budget aggregated by month.
   *
   * @param budgetId ID of the budget whose target and actual records to load
   * @return one record for each month from the current month to the first month that was booked in
   *     the given budget.
   */
  public List<AggregatedRecord> getMonthlyAggregationForBudget(long budgetId) {
    List<MonthlyAggregatedRecordBean> planRecords =
        planRecordRepository.aggregateByMonthAndBudget(budgetId);
    List<MonthlyAggregatedRecordBean> workRecords =
        workRecordRepository.aggregateByMonthAndBudget(budgetId);
    return recordJoiner.joinMonthly(workRecords, planRecords);
  }

  /**
   * Loads the actual budget burned in the given budget and the budget planned for all people active
   * in this budget aggregated by month with taxes.
   *
   * @param budgetId ID of the budget whose target and actual records to load
   * @return one record for each month from the current month to the first month that was booked in
   *     the given budget.
  ...
  public List<AggregatedRecord> getMonthlyAggregationForBudgetWithTax(long budgetId) {
    List<MonthlyAggregatedRecordWithTaxBean> planRecords =
        planRecordRepository.aggregateByMonthAndBudgetWithTax(budgetId);
    List<MonthlyAggregatedRecordWithTaxBean> workRecords =
        workRecordRepository.aggregateByMonthAndBudgetWithTax(budgetId);
    return recordJoiner.joinMonthlyWithTax(workRecords, planRecords);
  }

  /**
   * Loads the actual budget burned in a set of given budgets and the budget planned for all people
   * active in these budgets aggregated by week.
   *
  ...
  public List<AggregatedRecord> getWeeklyAggregationForBudgets(BudgetTagFilter budgetFilter) {
    ...
    if (budgetFilter.getSelectedTags().isEmpty()) {
      ...
      planRecords =
          planRecordRepository.aggregateByWeekAndBudgetTags(
              budgetFilter.getProjectId(), budgetFilter.getSelectedTags());
      workRecords =
          workRecordRepository.aggregateByWeekAndBudgetTags(
              budgetFilter.getProjectId(), budgetFilter.getSelectedTags());
    }
  ...
  /**
   * Loads the actual budget burned in a set of given budgets and the budget planned for all people
   * active in these budgets aggregated by week with taxes.
   *
   * @param budgetFilter filter that identifies the set of budgets whose data to load
   * @return one record for each week from the current week to the first week that was booked in the
   *     given budget
   */
  public List<AggregatedRecord> getWeeklyAggregationForBudgetsWithTaxes(
    ...
    if (budgetFilter.getSelectedTags().isEmpty()) {
      ...
      planRecords =
          planRecordRepository.aggregateByWeekForBudgetsWithTax(budgetFilter.getProjectId());
    } else {
      workRecords =
          workRecordRepository.aggregateByWeekAndPersonForBudgetsWithTax(
              budgetFilter.getProjectId(), budgetFilter.getSelectedTags());
  ...
  /**
   * Loads the actual budget burned in a set of given budgets and the budget planned for all people
   * active in these budget aggregated by month.
   *
   * @param budgetFilter filter that identifies the set of budgets whose data to load
   * @return one record for each month from the current month to the first month that was booked in
   *     the given budget.
  ...
  public List<AggregatedRecord> getMonthlyAggregationForBudgets(BudgetTagFilter budgetFilter) {
    ...
    if (budgetFilter.getSelectedTags().isEmpty()) {
      ...
      workRecords =
          workRecordRepository.aggregateByMonthAndBudgetTags(
              budgetFilter.getProjectId(), budgetFilter.getSelectedTags());
      planRecords =
          planRecordRepository.aggregateByMonthAndBudgetTags(
              budgetFilter.getProjectId(), budgetFilter.getSelectedTags());
    }
    return recordJoiner.joinMonthly(workRecords, planRecords);
  }
  ...
  /**
   * Loads the actual budget burned in a set of given budgets and the budget planned for all people
   * active in these budget aggregated by month with taxes.
   *
   * @param budgetFilter filter that identifies the set of budgets whose data to load
   * @return one record for each month from the current month to the first month that was booked in
   *     the given budget.
   */
  public List<AggregatedRecord> getMonthlyAggregationForBudgetsWithTax(
      BudgetTagFilter budgetFilter) {
    List<MonthlyAggregatedRecordWithTaxBean> planRecords;
    List<MonthlyAggregatedRecordWithTaxBean> workRecords;
    if (budgetFilter.getSelectedTags().isEmpty()) {
      ...
      workRecords =
          workRecordRepository.aggregateByMonthAndBudgetTagsWithTax(
              budgetFilter.getProjectId(), budgetFilter.getSelectedTags());
      planRecords =
          planRecordRepository.aggregateByMonthAndBudgetTagsWithTax(
              budgetFilter.getProjectId(), budgetFilter.getSelectedTags());
    }
    return recordJoiner.joinMonthlyWithTax(workRecords, planRecords);
  }
