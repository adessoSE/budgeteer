...
public class BudgeteerMethodSecurityExpressionRoot extends SecurityExpressionRoot implements MethodSecurityExpressionOperations {
     ...
     * @return <i>true</i>, if the id of the budget is valid and the
     ...
     * @return <i>true</i>, if the currently selected project has the same
     * id as the given one.
     * @see BudgeteerSession#getProjectId()
     */
    public boolean canReadProject(Long projectId) {
        if (BudgeteerSession.get().getLoggedInUser().isGlobalAdmin()) {
            return true;
        } else {
            return getCurrentProjectId() == projectId;
        }
    }

    /**
     * @param contractId The id of the contract to retrieve.
     * @return <i>true</i>, if the associated project of the contract has
     ...
     * @return <i>true</i>, if the associated contract's project has the
     ...
     * @return <i>true</i>, if the associated project of the person has
     * the same id as the currently selected project by the user.
     * @see BudgeteerSession#getProjectId()
     */
