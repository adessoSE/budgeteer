...
class InvoiceDataMapperTest {
	...
	/**
	 * Tests whether different invoiced associated with different contracts will get the same dynamic
	 * attributes
	 */
	@Test
	void testMapWithDifferentProjectsButSameAttributes() {
		...
		assertEquals(
				"contractInvoiceField1 Name", firstInvoice.getDynamicInvoiceFields().get(0).getName());
		assertEquals(
				"contractInvoiceField1 Value", firstInvoice.getDynamicInvoiceFields().get(0).getValue());
		assertEquals(
				"contractInvoiceField2 Name", firstInvoice.getDynamicInvoiceFields().get(1).getName());
		assertEquals(
				"contractInvoiceField2 Value", firstInvoice.getDynamicInvoiceFields().get(1).getValue());
		assertEquals(
				"contractInvoiceField3 Name", firstInvoice.getDynamicInvoiceFields().get(2).getName());
		...
		assertEquals(
				"contractInvoiceField1 Name", secondInvoice.getDynamicInvoiceFields().get(0).getName());
		assertEquals("", secondInvoice.getDynamicInvoiceFields().get(0).getValue());
		assertEquals(
				"contractInvoiceField2 Name", secondInvoice.getDynamicInvoiceFields().get(1).getName());
		assertEquals("", secondInvoice.getDynamicInvoiceFields().get(1).getValue());
		assertEquals(
				"contractInvoiceField3 Name", secondInvoice.getDynamicInvoiceFields().get(2).getName());
		assertEquals(
				"contractInvoiceField3 Value", secondInvoice.getDynamicInvoiceFields().get(2).getValue());
	}
	...
	void testMap() {
		...
		assertEquals(
				"contractInvoiceField1 Name", mappedElement.getDynamicInvoiceFields().get(0).getName());
		assertEquals(
				"contractInvoiceField1 Value", mappedElement.getDynamicInvoiceFields().get(0).getValue());
		assertEquals(
				"contractInvoiceField2 Name", mappedElement.getDynamicInvoiceFields().get(1).getName());
		assertEquals(
				"contractInvoiceField2 Value", mappedElement.getDynamicInvoiceFields().get(1).getValue());
	}
