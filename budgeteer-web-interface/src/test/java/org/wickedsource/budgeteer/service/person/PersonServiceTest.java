...
class PersonServiceTest extends ServiceTestTemplate {
  ...
  void testLoadPeopleBaseData() {
    when(personRepository.findBaseDataByProjectId(1L))
        .thenReturn(Arrays.asList(createPersonBaseDataBean()));
    ...
    Assertions.assertEquals(
        MoneyUtil.createMoneyFromCents(10000L), data.get(0).getAverageDailyRate());
  ...
  private PersonBaseDataBean createPersonBaseDataBean() {
    return new PersonBaseDataBean(
        1L, "person1", 500000000L, 50000L, fixedDate, MoneyUtil.createMoney(10));
  }

  private PersonDetailDataBean createPersonDetailDataBean() {
    return new PersonDetailDataBean(
        1L, "person1", 6172800000L, 50000L, fixedDate, fixedDate, 5.0d, 654321L);
  }
}
