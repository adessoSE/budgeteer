...
class ContractServiceTest {
	...
	void testSaveNewContract() {
		...
		assertEquals(
				5, projectRepository.findById(savedContract.getProjectId()).getContractFields().size());
	}
	...
	void testUpdateContract() {
		...
		assertEquals(MoneyUtil.createMoney(12), savedContract.getBudget());
		assertEquals(
				DateUtils.truncate(new Date(), Calendar.DAY_OF_MONTH), savedContract.getStartDate());
		...
		assertEquals(
				6, projectRepository.findById(savedContract.getProjectId()).getContractFields().size());
	}

	/**
	 * Update a Contract that does not has dynamic fields, but is associated with a project that has
	 * already ProjectContractFields
	 */
	@Test
	@DatabaseSetup("contractTest.xml")
	@DatabaseTearDown(value = "contractTest.xml", type = DatabaseOperation.DELETE_ALL)
	void testUpdateContract1() {
		...
		assertEquals(
				7, projectRepository.findById(savedContract.getProjectId()).getContractFields().size());
	}
