...
class TemplateServiceTest {
	...
	void doImportTest() {
		...
		Assertions.assertEquals(
				testDto.getName(), templateService.getTemplatesInProject(1).get(0).getName());
		Assertions.assertEquals(
				"TEST_D", templateService.getTemplatesInProject(1).get(0).getDescription());
	...
	void editTemplateTest() {
		...
		long newId =
				templateService.editTemplate(
						1,
						templateService.getTemplatesInProject(1).get(0).getId(),
						new ImportFile(
								"exampleTemplate1.xlsx", getClass().getResourceAsStream("exampleTemplate1.xlsx")),
						model(from(testDto)));
	...
	void getExampleFileTest() {
		try {
			XSSFWorkbook testWorkbok =
					(XSSFWorkbook) WorkbookFactory.create(templateService.getExampleFile().getInputStream());
