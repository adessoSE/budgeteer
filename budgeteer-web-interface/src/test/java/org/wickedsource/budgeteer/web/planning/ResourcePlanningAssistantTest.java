...
class ResourcePlanningAssistantTest {
	...
	void testAllocate() throws Exception {
		...
		Person fullTimePerson =
				new Person("Felix Fulltime", new Percent(100), MoneyUtil.createMoneyFromCents(50000));
		fullTimePerson.addAbsence(
				new TimePeriod(
						getDate(2015, Calendar.JULY, 6),
						getDate(2015, Calendar.JULY, 10))); // 5 working days missing
		fullTimePerson.addAbsence(
				new TimePeriod(
						getDate(2015, Calendar.SEPTEMBER, 4),
						getDate(2015, Calendar.SEPTEMBER, 7))); // 2 working days missing
		Person partTimePerson =
				new Person("Paul Parttime", new Percent(50), MoneyUtil.createMoneyFromCents(60000));
