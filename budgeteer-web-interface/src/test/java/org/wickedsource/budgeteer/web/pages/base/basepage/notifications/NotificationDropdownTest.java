...
class NotificationDropdownTest extends AbstractWebTestTemplate {
  ...
  void testRenderFull() {
    ...
    when(notificationService.getNotifications(1L, 1L)).thenReturn(createNotifications());
  ...
  void testRenderEmpty() {
    WicketTester tester = getTester();
    when(notificationService.getNotifications(1L, 1L)).thenReturn(EMPTY_NOTIFICATION_LIST);
    NotificationModel model = new NotificationModel(1L, 1L);
    NotificationDropdown dropdown = new NotificationDropdown("dropdown", model);
    tester.startComponentInPage(dropdown);
