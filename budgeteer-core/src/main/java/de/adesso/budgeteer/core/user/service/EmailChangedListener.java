...
public class EmailChangedListener
    implements ApplicationListener<OnEmailChangedEvent>, ResendVerificationTokenUseCase {
  ...
  public void resendVerificationToken(long userId)
      throws EmailAlreadyVerifiedException, MailNotEnabledException {
    if (!emailEnabled) {
  ...
  private void sendVerificationToken(long userId, String name, String email) {
    ...
    createVerificationTokenPort.createVerificationToken(
        userId, token, LocalDateTime.now().plusHours(24));
