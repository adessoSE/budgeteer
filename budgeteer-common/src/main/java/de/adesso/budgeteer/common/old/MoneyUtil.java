package de.adesso.budgeteer.common.old;
...
/**
 * @deprecated Old MoneyUtil class with some questionable functions. Use the new {@link
 *     de.adesso.budgeteer.common.money.MoneyUtil} instead. If you need one of the functions here,
 *     check if it's sensible and add it to the new class.
 */
...
public class MoneyUtil {
  ...
  public static Double toDouble(Money money, Double unit, Double taxRate) {
    return money
        .dividedBy(unit, RoundingMode.FLOOR)
        .multipliedBy(taxRate, RoundingMode.FLOOR)
        .getAmount()
        .doubleValue();
  }
  ...
  public static long getCentsByHourFraction(
      long monthValueInCents, Double monthHours, Double weekHours) {
